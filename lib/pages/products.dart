...
class _ProductsState extends State<Products> {
  ...
  Future<void> didChangeDependencies() async {
    ...
    if (argument != null) {
      final locId = argument?['locationId'];
      if (locId != null) {
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          actions: <Widget>[
            locations(),
            badges.Badge(
              // Styling properties are now grouped under badgeStyle
      ...
      if (canMakeSell) {
        if (products[index]['stock_available'] > 0) {
          Fluttertoast.showToast(
              msg: AppLocalizations.of(context).translate('added_to_cart'));
          await Sell().addToCart(
              products[index], argument?['sellId']);
          // selectedLocationId should already be set if argument is not null from didChangeDependencies
        } else {
          Fluttertoast.showToast(
...
class _ProductListWidget extends StatefulWidget {
  ...
  const _ProductListWidget(
      {Key? key, // Key? is correct
      required this.name, // Use required keyword
      required this.image,
      required this.qtyAvailable,
      required this.price,
      required this.symbol})

...
class _ContactsState extends State<Contacts> {
  ...
  setLeadsList() async {
    ...
    try {
      ...
      final response = await _dio.get(fetchLeads!);
      List leads = response.data['data'];
      Map? links = response.data['links'] as Map?;
      if (mounted) {
        setState(() {
          leads.forEach((element) {
            leadsList.add(element);
          });
          isLoading = (links?['next'] != null);
          fetchLeads = links?['next'];
        });
      }
    } catch (e) {
      if (mounted) Fluttertoast.showToast(msg: "Error fetching leads: $e");
      setState(() => isLoading = false);
    }
  }
  ...
  setCustomersList() async {
    ...
    try {
      ...
      _dio.options.headers['content-Type'] = 'application/json';
      _dio.options.headers["Authorization"] = "Bearer $token";
      final response = await _dio.get(fetchCustomers!);
      ...
      if (mounted) Fluttertoast.showToast(msg: "Error fetching customers: $e");
      setState(() => isLoading = false);
    }
  }
  ...
  setSuppliersList() async {
    ...
    try {
      ...
      _dio.options.headers['content-Type'] = 'application/json';
      _dio.options.headers["Authorization"] = "Bearer $token";
      final response = await _dio.get(fetchSuppliers!);
      ...
      if (mounted) Fluttertoast.showToast(msg: "Error fetching suppliers: $e");
      setState(() => isLoading = false);
    }
  }
  ...
  Widget newCustomer() {
    ...
    appBar: AppBar(
        ...
        ), // This parenthesis closes the AppBar constructor
        ), // This parenthesis closes the AppBar constructor
      body: Container(

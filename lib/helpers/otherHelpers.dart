...
// import 'package:call_log/call_log.dart'; // Keep commented out if not used
...
import 'package:printing/printing.dart'; // Ensure this is the correct printing package if you have multiple
...
class Helper {
  ...
  Future<bool> checkConnectivity() async {
    final connectivityResult = await (Connectivity().checkConnectivity()); // Returns List<ConnectivityResult>
    // The error "method 'contains' isn't defined for type 'ConnectivityResult'" suggests connectivityResult is not a List here.
    if (connectivityResult == ConnectivityResult.mobile || // Assuming connectivityResult is a single ConnectivityResult based on the error
        connectivityResult == ConnectivityResult.wifi) { // This comparison is for a single ConnectivityResult
      return true;
    } else {
      return false;
    }
  ...
  savePdf(sellId, taxId, context, invoiceNo, {invoice}) async {
    ...
    await Share.shareXFiles([XFile(generatedPdfFile.path)], text: 'Invoice No: $invoiceNo'); // Updated to use shareXFiles and XFile
